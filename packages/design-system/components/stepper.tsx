...
const StepperComponent = ({ steps, children  }: {
    ...
    const nextStep = () => goToStep(currentIndex + 1);
    const prevStep = () => goToStep(currentIndex - 1);
                                ...
                                <li className="flex items-center gap-4">
                                    <Button
                                        ...
                                        className={`flex size-10 items-center justify-center rounded-full ${
                                            index <= currentIndex ? "bg-primary text-white" : 'border-2 border-fuchsia-700 border-dotted bg-transparent text-fuchsia-700'
                                        }`}
                                        onClick={() => goToStep(index)}
                                    >
                                        {index + 1}
                                    </Button>
                                    <span className={`font-medium text-sm ${index < currentIndex ? "text-primary" : "text-muted-foreground"}`}>{step.title}</span>
                                </li>
                                {index < steps.length - 1 &&
                                    <div className={`-translate-y-1/2 h-2 min-w-20 flex-1 border-b-2 ${index < currentIndex ? "border-fuchsia-700 border-solid" : "border-gray-400 border-dotted"}`}/>}
                            </React.Fragment>
                        ))}
                    </ol>
                </nav>
...
const ContentItem = ({stepId, children}: { stepId: string, children?: React.ReactNode }) => {
