...
const ImageCard = React.forwardRef<
    HTMLDivElement,
    React.HTMLAttributes<HTMLDivElement> & { imageUrl: string , cardFull?: boolean }
>(({ className, imageUrl, cardFull, ...props }, ref) => (
    <div
        ref={ref}
        className={cn(
            'relative min-h-full flex overflow-hidden rounded-xl border bg-card text-card-foreground shadow',
            className,
        )}
        {...props}
    >
        ...
        <div className={`relative flex flex-col w-full h-auto px-12 py-8 gap-5 lg:px-24 lg:py-16 lg:gap-10 justify-between ${cardFull ? 'lg:flex-row' : 'lg:flex-col'}`}>
            {props.children}
        </div>
    </div>
))
...
>(({ className, ...props }, ref) => (
    <div
        ref={ref}
        className={cn('font-semibold text-center text-3xl text-white', className)}
        {...props}
    />
))
...
const ImageCardContent = React.forwardRef<
    ...
    React.HTMLAttributes<HTMLDivElement>
>(({ className, ...props }, ref) => (
    <div ref={ref} className={cn('m-0 pt-0 text-center text-white text-xl', className)} {...props} />
))
