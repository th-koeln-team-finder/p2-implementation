...
export const userSkills = pgTable(
    ...
    (userSkill) => ({
        validLevel: check('valid_userSkill_level', sql`${userSkill.level}
        >= 0`),
    }),
)
...
export const userInvitations = pgTable(
    ...
    (userInvitations) => ({
        userInvitationUniqueConstraint: unique(
            'userInvitationUniqueConstraint',
        ).on(userInvitations.projectId, userInvitations.userId),
    }),
)
