...
export function ProjectIssuesList({
    ...
    return (
            ...
            <div className="flex-col justify-start items-start gap-4 inline-flex">
                ...
                <div
                    className="self-stretch text-2xl font-medium leading-loose">{'issuesTitle'}
                </div>
                <div className="overflow-hidden transition-all duration-300 ease-in-out " style={{maxHeight}}>

                    {listOfIssues.map((issue,index)=> (
                        <Card key={issue.id} ref={index === 0 ? itemRef : null} className={`w-full self-stretch justify-between inline-flex mb-2 opacity-0 transition-opacity duration-300 ${showAll || index < 4 ? "opacity-100" : ""}`}>
