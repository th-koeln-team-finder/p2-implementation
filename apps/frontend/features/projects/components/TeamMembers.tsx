...
/*
const carouselItems = participants
    .map((participant) => (
        <CarouselItem key={participant.users.id} className="flex basis-1/3 flex-col md:basis-1/6">
...
export default function TeamMembers({
    ...
    participants: { users: UserSelect }[]

}) {
    const t = useTranslations('projects')

    const carouselItems = participants
        .map((participant, i) => `${participant.users.name}`)
        .map((v) => (
